...
DEFAULT_DHDT    = 0.0       # dH/dt                        [W m-2]
DEFAULT_DRDT    = 0.0       # dR/dt                        [W m-2]
DEFAULT_DRAG_Q  = 0.01      # drag coefficient for moisture [m s-1]
...
def main():
    ...
    # --- initialise -----------------------------------------------------------
    info = LPJmLCoupler.lpjml_init(config)
    ncells    = info["ncells"]
    ...
    # Climate inputs (prec, temp, swdown, lwnet, wind) are passed as zeros
    # because driver.c calls dailyclimate() internally, which overwrites them
    # with values read from the files configured in the .cjson.
    prec_in      = np.zeros(ncells)
    temp_mean_in = np.zeros(ncells)
    swdown_in    = np.zeros(ncells)
    lwnet_in     = np.zeros(ncells)
    wind_in      = np.zeros(ncells)
    co2_in       = np.zeros(ncells)   # ignored when CO2 is read from file

    # FMS coupling variables: no file equivalent, use neutral defaults.
    ...
    # --- time loop ------------------------------------------------------------
    # dt_fast = NSECONDSDAY (86400 s) so every call triggers a full day of
    # physics (goodmorning and goodnight are both true on every lpjml_update).
    for year in [firstyear]:#range(firstyear - nspinup, lastyear + 1):
